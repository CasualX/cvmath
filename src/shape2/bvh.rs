...
struct BvhBranch<T> {
	left_bounds: Bounds2<T>,
	right_bounds: Bounds2<T>,
	left_child: u32,
	right_child: u32,
}
...
impl<T: Float> BvhBranch<T> {
	const EMPTY: BvhBranch<T> = BvhBranch {
		left_bounds: Bounds2::EMPTY,
		right_bounds: Bounds2::EMPTY,
		left_child: 0,
		right_child: 0,
	};
}
...
/// }
/// impl Collection {
/// 	pub fn build(shapes: Vec<Shape2<f32>>) -> Collection {
/// 		let bvh = Bvh2::build(shapes.iter().map(|shape| shape.bounds().unwrap()).enumerate());
...
impl<T> Bvh2<T> {
	...
	pub fn build<I>(items: I) -> Bvh2<T> where T: Float, I: IntoIterator<Item = (usize, Bounds2<T>)> {
...
impl<T: Float> Bvh2<T> {
	...
	#[inline]
	pub fn visit<F>(&self, mut f: F) where F: FnMut(u32, &Bounds2<T>) -> bool {
		self.visit_node(0, 1, &mut f)
	}
	fn visit_node<F>(&self, node_index: u32, depth: u32, f: &mut F) where F: FnMut(u32, &Bounds2<T>) -> bool {
	...
	fn inside_node<F>(&self, node_index: u32, pt: Point2<T>, f: &mut F) -> bool where F: FnMut(usize, Point2<T>) -> bool {
		let Some(node) = self.nodes.get(node_index as usize) else {
			return false;
		};
	...
	#[inline]
	fn inside_child<F>(&self, bounds: Bounds2<T>, child: u32, pt: Point2<T>, f: &mut F) -> bool where F: FnMut(usize, Point2<T>) -> bool {
	...
	#[inline]
	fn inside_leaf<F>(&self, leaf: &BvhLeaf, pt: Point2<T>, f: &mut F) -> bool where F: FnMut(usize, Point2<T>) -> bool {
	...
	#[inline]
	pub fn trace<F>(&self, ray: &Ray2<T>, mut f: F) -> Option<Hit2<T>> where F: FnMut(usize, &Ray2<T>) -> Option<Hit2<T>> {
		let mut ray = BvhRay::new(ray);
		self.trace_node(0, &mut ray, &mut f)
	}

	fn trace_node<F>(&self, node_index: u32, ray: &mut BvhRay<T>, f: &mut F) -> Option<Hit2<T>> where F: FnMut(usize, &Ray2<T>) -> Option<Hit2<T>> {
	...
	#[inline]
	fn trace_branch<F>(&self, branch: &BvhBranch<T>, ray: &mut BvhRay<T>, f: &mut F) -> Option<Hit2<T>> where F: FnMut(usize, &Ray2<T>) -> Option<Hit2<T>> {
	...
	#[inline]
	fn trace_leaf<F>(&self, leaf: &BvhLeaf, ray: &mut BvhRay<T>, f: &mut F) -> Option<Hit2<T>> where F: FnMut(usize, &Ray2<T>) -> Option<Hit2<T>> {
		let mut best = None;
...
#[inline]
fn ray_aabb<T: Float>(ray: &BvhRay<T>, bounds: &Bounds2<T>) -> Option<T> {
	#[inline]
	...
	let near = T::max(T::max(x0, y0), ray.ray.distance.min);
	let far = T::min(T::min(x1, y1), ray.ray.distance.max);
...
struct BvhRay<T> {
	ray: Ray2<T>,
	inv_direction: Vec2<T>,
}

impl<T: Float> BvhRay<T> {
	#[inline]
	fn new(ray: &Ray2<T>) -> BvhRay<T> {
		BvhRay {
			ray: ray.clone(),
...
fn build_node<T: Float>(nodes: &mut Vec<BvhNode<T>>, items: &mut [BuildItem<T>]) -> (usize, Bounds2<T>) {
...
#[inline]
fn union_bounds<T: Float>(items: &[BuildItem<T>]) -> Bounds2<T> {
...
fn optimize_reorder<T, U: Clone>(this: &mut Bvh2<T>, items: Vec<U>) -> Vec<U> {
